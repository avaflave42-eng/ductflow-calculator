...
/**
 * A15G: Rectangular Exit without Blades
 * Inputs: H, W, angle, Q
 * Uses angle matching (round up)
 */
export function A15G_calc(inputs: CalcInputs, data: MasterData): CalcOutputs {
  ...
  const angle_vals = [...new Set(a15g_rows.map((r) => r.ANGLE))].sort((a, b) => a - b);
  const angle_match = angle_vals.filter((v) => v >= angle).shift() || angle_vals[angle_vals.length - 1];

  const matched_row = a15g_rows.find((row) => row.ANGLE === angle_match);
