...
/**
 * A11O: Diverging Junction - Rectangular Tee
 * Inputs: H main, W main, H branch, W branch, Qc, Qb
 * Uses Vb/Vc and Qb/Qc dual matching for branch
 */
export function A11O_calc(inputs: CalcInputs, data: MasterData): CalcOutputs {
  ...
  // --- BRANCH CALCULATIONS ---
  const branch_data = data.rows.filter((row) => row.id === "A11O" && row.PATH === "branch");
