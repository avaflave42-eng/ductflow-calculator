...
/**
 * A11G: Diverging Junction - Round Tee
 * Inputs: D main, D branch, Qc, Qb
 * Uses Vb/Vc matching for branch
 */
export function A11G_calc(inputs: CalcInputs, data: MasterData): CalcOutputs {
  ...
  // --- BRANCH CALCULATIONS ---
  const branch_data = data.rows.filter((row) => row.id === "A11G" && row.PATH === "branch");
  const Vb_Vc = Vb / Vc;
