...
/**
 * A8H: Rectangular Expansion (Configuration H)
 * Inputs: H, H₁, W, Angle, Flow Rate
 * Simple angle matching (always ANGLE >= entry_4)
 */
export function A8H_calc(inputs: CalcInputs, data: MasterData): CalcOutputs {
  ...
  const a8h_rows = data.rows.filter((row) => row.id === "A8H");

  // Match angle (ANGLE >= entry_4)
  const valid_angle_rows = a8h_rows.filter((row) => row.ANGLE >= entry_4);
  const closest_angle = valid_angle_rows.length > 0
    ? Math.min(...valid_angle_rows.map((row) => row.ANGLE))
    : Math.max(...a8h_rows.map((row) => row.ANGLE));

  const angle_filtered = a8h_rows.filter((row) => row.ANGLE === closest_angle);
