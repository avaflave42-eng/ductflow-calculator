...
/**
 * A11B: Diverging Junction - Round Conical Tee
 * Inputs: D main, D branch, Qc, Qb
 * Uses Vb/Vc matching for branch
 */
export function A11B_calc(inputs: CalcInputs, data: MasterData): CalcOutputs {
  ...
  // --- BRANCH CALCULATIONS ---
  const branch_data = data.rows.filter((row) => row.id === "A11B" && row.PATH === "branch");

  const Vb_Vc = Vb / Vc;

  // Find closest Vb/Vc match
