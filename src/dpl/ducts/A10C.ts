...
/**
 * A10C: Converging Junction - Rectangular (45° branch, Ab = Ac/2)
 * Inputs: Main Height, Main Width, Q source, Q branch
 * Outputs: Branch and Main calculations
 * Error check: Qb/Qs >= 0.4
 */
export function A10C_calc(inputs: CalcInputs, data: MasterData): CalcOutputs {
  ...
  // --- BRANCH CALCULATIONS (uses A10C data with PATH = "branch") ---
  const branch_data = data.rows.filter((row) => row.id === "A10C" && row.PATH === "branch");
