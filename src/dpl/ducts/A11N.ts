...
/**
 * A11N: Diverging Junction - Rectangular Tee
 * Inputs: H main, W main, H branch, W branch, Qc, Qb
 * Uses Vb/Vc and Qb/Qc dual matching for branch
 */
export function A11N_calc(inputs: CalcInputs, data: MasterData): CalcOutputs {
  ...
  // --- BRANCH CALCULATIONS ---
  const branch_data = data.rows.filter((row) => row.id === "A11N" && row.PATH === "branch");

  const branch_with_diff = branch_data.map((row) => ({
    row,
    vb_vc_diff: Math.abs(row["Vb/Vc"] - Vb_Vc),
    qb_qc_diff: Math.abs(row["Qb/Qc"] - Qb_Qc),
  }));
