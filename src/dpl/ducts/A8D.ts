...
/**
 * A8D: Rectangular to Round Transition
 * Inputs: H, W, D, L, Flow Rate
 * Calculates angle from geometry, uses A8B lookup table
 */
export function A8D_calc(inputs: CalcInputs, data: MasterData): CalcOutputs {
  ...
  // Velocity based on rectangular area
  const velocity = entry_5 / area_rect; // ft/min

  // Calculate angle (contraction formula)
  const tan_half_theta = (entry_3 - 1.13 * Math.sqrt(entry_1 * entry_2)) / (2 * entry_4);
  const theta_deg = (2 * Math.atan(tan_half_theta) * 180) / Math.PI;

  // Area ratio A₁/A
  const area_ratio = area_round / area_rect;
