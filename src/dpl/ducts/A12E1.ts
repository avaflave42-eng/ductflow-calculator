...
/**
 * A12E1: Conical Converging Bellmouth with End Wall (Round)
 * Inputs: L, D, Ds, Q, obstruction, n
 * Uses L/D and angle matching with optional screen correction
 */
export function A12E1_calc(inputs: CalcInputs, data: MasterData): CalcOutputs {
  ...
  // --- BASE COEFFICIENT LOOKUP (uses A12E1 data) ---
  const base_data = data.rows.filter((row) => row.id === "A12E1");
