...
/**
 * A12C: Bellmouth Entry (Round) - Alternative
 * Inputs: R, D, Ds, Q, obstruction, n
 * Uses R/D matching with optional screen obstruction correction
 */
export function A12C_calc(inputs: CalcInputs, data: MasterData): CalcOutputs {
  ...
  // --- BASE COEFFICIENT LOOKUP (uses A12C data) ---
  const base_data = data.rows.filter((row) => row.id === "A12C");
