...
/**
 * A14B2: Perforated Plate (Rectangular Duct)
 * Inputs: H, W, Q, n, t, d_hole
 * Uses n and t/D matching (both round down)
 */
export function A14B2_calc(inputs: CalcInputs, data: MasterData): CalcOutputs {
  ...
  const t = inputs.entry_5 as number;
  const d_hole = inputs.entry_6 as number;
  ...
  const a14b2_rows = data.rows.filter((row) => row.id === "A14B2");
  const n_vals = [...new Set(a14b2_rows.map((r) => r["n, free area ratio"]))].sort((a, b) => a - b);
  const tD_vals = [...new Set(a14b2_rows.map((r) => r["t/D"]))].sort((a, b) => a - b);

  const n_match = n_vals.filter((v) => v <= n).pop() || n_vals[0];
  const tD_match = tD_vals.filter((v) => v <= t_D).pop() || tD_vals[0];

  const matched_row = a14b2_rows.find(
