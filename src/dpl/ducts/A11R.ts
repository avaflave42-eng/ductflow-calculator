...
/**
 * A11R: Diverging Junction - Rectangular
 * Inputs: H main, W main, H branch, W branch, Qc, Qb
 * Uses filtering logic where row values must be >= calculated ratios
 */
export function A11R_calc(inputs: CalcInputs, data: MasterData): CalcOutputs {
  ...
  // --- BRANCH CALCULATIONS ---
  const branch_data = data.rows.filter((row) => row.id === "A11R" && row.PATH === "branch");

  const branch_with_diff = branch_data.map((row) => {
    const row_vb_vc = row["Vb/Vc"];
    const row_qb_qc = row["Qb/Qc"];

    // Filter: use only rows where values >= calculated
    const vb_vc_diff = row_vb_vc >= Vb_Vc ? Math.abs(row_vb_vc - Vb_Vc) : Infinity;
    const qb_qc_diff = row_qb_qc >= Qb_Qc ? Math.abs(row_qb_qc - Qb_Qc) : Infinity;
