...
/**
 * A11S: Diverging Junction - Round Branch to Rectangular Main
 * Inputs: H main, W main, D branch, Qc, Qb
 * Uses Vb/Vc and Qb/Qc matching for branch
 */
export function A11S_calc(inputs: CalcInputs, data: MasterData): CalcOutputs {
  ...
  const A_branch = (Math.PI * Math.pow(D_branch / 12, 2)) / 4;
  ...
  // --- BRANCH CALCULATIONS ---
  const branch_data = data.rows.filter((row) => row.id === "A11S" && row.PATH === "branch");

  const branch_with_diff = branch_data.map((row) => {
    const row_vb_vc = row["Vb/Vc"];
    const row_qb_qc = row["Qb/Qc"];

    // Vb/Vc: use rows >= calculated value
    const vb_vc_diff = row_vb_vc >= Vb_Vc ? Math.abs(row_vb_vc - Vb_Vc) : Infinity;
    // Qb/Qc: simple absolute difference
    const qb_qc_diff = Math.abs(row_qb_qc - Qb_Qc);
