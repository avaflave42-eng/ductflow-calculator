...
/**
 * A9A2: Rectangular Pyramidal Contraction
 * Inputs: H, W, H₁, W₁, Angle, Flow Rate
 * Velocity based on downstream (smaller) dimensions
 */
export function A9A2_calc(inputs: CalcInputs, data: MasterData): CalcOutputs {
  const entry_1 = inputs.entry_1 as number; // H - Height (inches)
  const entry_2 = inputs.entry_2 as number; // W - Width (inches)
  const entry_3 = inputs.entry_3 as number; // H₁ - Height 1 (inches, downstream)
  const entry_4 = inputs.entry_4 as number; // W₁ - Width 1 (inches, downstream)
  ...
  const velocity = entry_6 / area_1; // ft/min
  ...
  const a9a2_rows = data.rows.filter((row) => row.id === "A9A2");

  // Match angle (ANGLE >= entry_5)
  const valid_angle_rows = a9a2_rows.filter((row) => row.ANGLE >= entry_5);
  const closest_angle = valid_angle_rows.length > 0
    ? Math.min(...valid_angle_rows.map((row) => row.ANGLE))
    : Math.max(...a9a2_rows.map((row) => row.ANGLE));

  const angle_filtered = a9a2_rows.filter((row) => row.ANGLE === closest_angle);
