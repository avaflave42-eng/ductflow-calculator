...
/**
 * A12A2: Rectangular Duct Entry
 * Inputs: t, L, H, W, Q, obstruction, n, plate_thickness, hole_diameter
 * Uses equivalent diameter for t/D and L/D matching with optional obstruction correction
 */
export function A12A2_calc(inputs: CalcInputs, data: MasterData): CalcOutputs {
  const t = inputs.entry_1 as number;
  const L = inputs.entry_2 as number;
  const H = inputs.entry_3 as number;
  const W = inputs.entry_4 as number;
  const Q = inputs.entry_5 as number;
  const obstruction = inputs.entry_6 as string;
  const n = inputs.entry_7 as number | undefined;
  const plate_thickness = inputs.entry_8 as number | undefined;
  const hole_diameter = inputs.entry_9 as number | undefined;

  // Calculate area and equivalent diameter
  const A_in2 = H * W;
  const A = A_in2 / 144; // ft²
  const D_equiv = 1 / (2 * (H + W) / (H * W)); // in
  const V = Q / A; // fpm
  ...
  const base_data = data.rows.filter((row) => row.id === "A12A2");
