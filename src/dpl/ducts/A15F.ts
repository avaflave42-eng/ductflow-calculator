...
/**
 * A15F: Rectangular Exit with Opposed Blades
 * Inputs: H, W, N (number of blades), angle, Q
 * Uses L/R and angle matching (both round up)
 */
export function A15F_calc(inputs: CalcInputs, data: MasterData): CalcOutputs {
  ...
  const a15f_rows = data.rows.filter((row) => row.id === "A15F");
  const LR_vals = [...new Set(a15f_rows.map((r) => r["L/R"]))].sort((a, b) => a - b);
  const angle_vals = [...new Set(a15f_rows.map((r) => r.ANGLE))].sort((a, b) => a - b);
  ...
  const matched_row = a15f_rows.find(
