...
/**
 * A8J: Rectangular Expansion (Configuration J)
 * Inputs: H, W, H₁, W₁, Angle, Flow Rate
 * Full rectangular expansion with both dimensions changing
 */
export function A8J_calc(inputs: CalcInputs, data: MasterData): CalcOutputs {
  ...
  const a8j_rows = data.rows.filter((row) => row.id === "A8J");

  // Match angle (ANGLE >= entry_5)
  const valid_angle_rows = a8j_rows.filter((row) => row.ANGLE >= entry_5);
  const closest_angle = valid_angle_rows.length > 0
    ? Math.min(...valid_angle_rows.map((row) => row.ANGLE))
    : Math.max(...a8j_rows.map((row) => row.ANGLE));

  const angle_filtered = a8j_rows.filter((row) => row.ANGLE === closest_angle);
