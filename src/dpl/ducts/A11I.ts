...
/**
 * A11I: Diverging Junction - Round Tee
 * Inputs: D main, D branch, Qc, Qb
 * Uses Vb/Vc matching for branch
 */
export function A11I_calc(inputs: CalcInputs, data: MasterData): CalcOutputs {
  ...
  // --- BRANCH CALCULATIONS ---
  const branch_data = data.rows.filter((row) => row.id === "A11I" && row.PATH === "branch");
  const Vb_Vc = Vb / Vc;
